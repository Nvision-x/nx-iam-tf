...
# ------------------- Knowledge Hub data-plane access ---------------------

variable "enable_knowledge_hub_neptune_access" {
  description = "Attach a neptune-db:* connect policy to the knowledge-hub role"
  type        = bool
  default     = false
}

variable "knowledge_hub_neptune_cluster_resource_id" {
  description = "Neptune cluster_resource_id to scope the connect policy to. Empty means all Neptune clusters in the account, since the id is generated by AWS and cannot be derived from the cluster identifier."
  type        = string
  default     = ""
}

variable "knowledge_hub_neptune_policy_name" {
  description = "Override the Neptune connect policy name. Set this to the existing name when migrating the policy out of nx-infra-tf so the import matches."
  type        = string
  default     = ""
}

variable "enable_knowledge_hub_text_cache_access" {
  description = "Attach an s3:GetObject policy for the text content cache bucket to the knowledge-hub role"
  type        = bool
  default     = false
}
...
variable "knowledge_hub_text_cache_policy_name" {
  description = "Override the text cache policy name. Set this to the existing name when migrating the policy out of nx-infra-tf so the import matches."
  type        = string
  default     = ""
}

# ------------------- Monitoring chatbot role -----------------------------

variable "enable_monitoring_chatbot_role" {
  description = "Create the AWS Chatbot service role consumed by nx-infra-tf's Slack channel configuration"
  type        = bool
  default     = false
}

variable "monitoring_chatbot_role_name" {
  description = "Override the chatbot role name, defaults to <cluster_name>-chatbot-alarms. Set this to the existing name when migrating the role out of nx-infra-tf."
  type        = string
  default     = ""
}
